...
class WalletConnect {
  ...
  func connect() -> String {
    ...
    let wcUrl = WCURL(topic: UUID().uuidString,
                      ...
                      key: try! randomKey())
    let clientMeta = Session.ClientMeta(name: "PPAP",
                                        description: "WalletConnectSwift",
                                        icons: [],
                                        url: URL(string: "https://ppap.com")!)
  ...
  func reconnectIfNeeded() {
    if let oldSessionObject = UserDefaults.standard.object(forKey: Self.sessionKey) as? Data,
...
extension WalletConnect: ClientDelegate {
  ...
  func client(_ client: Client, didConnect session: Session) {
    ...
    print("USER DEFAULTS: \(Self.sessionKey)", session)
    delegate.didConnect()
  }
